...
uint8_t left_last_pos = 0;
uint8_t right_last_pos = 0;
  ...
  while(1) {
    ...
    // #endif
      ...
      //cmd2 = CLAMP(adc_buffer.l_rx2 - ADC2_MIN, 0, ADC2_MAX) / (ADC2_MAX / 1000.0f);  // ADC2
	    ...
	    cmd1 = (adc_buffer.l_tx2 - ((ADC1_MIN+ADC1_MAX)/2.0))/ (((ADC1_MIN+ADC1_MAX)/2)/ 1000.0f);  // ADC1
      cmd2 = (adc_buffer.l_rx2 - ((ADC2_MIN+ADC2_MAX)/2.0))/ (((ADC2_MIN+ADC2_MAX)/2) / 1000.0f);  // ADC2
    ...
    speedL = CLAMP(steer * STEER_COEFFICIENT, -1000, 1000);
    ...
    if(left_last_pos != halll){
      // HAL_GPIO_WritePin(INTERRUPT_PORT, LEFT_INTERRUPT_PIN,1);
      HAL_GPIO_TogglePin(INTERRUPT_PORT,LEFT_INTERRUPT_PIN);
      left_last_pos = halll; 
      // HAL_Delay(1);
    }

    if(right_last_pos != hallr){
      // HAL_GPIO_WritePin(INTERRUPT_PORT, RIGHT_INTERRUPT_PIN,1);
      HAL_GPIO_TogglePin(INTERRUPT_PORT,RIGHT_INTERRUPT_PIN);
      right_last_pos = hallr; 
      // HAL_Delay(1);
    }
    // HAL_GPIO_WritePin(INTERRUPT_PORT, LEFT_INTERRUPT_PIN,1);
    // HAL_GPIO_WritePin(INTERRUPT_PORT, RIGHT_INTERRUPT_PIN,1);
